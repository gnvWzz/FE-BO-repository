...
export default function AddManufacturerImage() {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      ...
      .then(
        ...
        axios
          .post(`http://localhost:8080/manufacturer/add-image/${manufacturerId}`, 
            imageUrl
          )
          .then((response) => {
          ...
          .catch((error) => {
            ...
            console.log("Error when setting manufacturer image ", error);
          });
      }
      )
      .catch((error) => {
 ...
 useEffect(() => {
  ...
  <Box m="20px">
    <Header
      title="UPLOAD MANUFACTURER AVATAR"
      subtitle="Upload the avatar of potential manufacturer"
    />
    <Box style={{ 
      ...
      <Box style={{paddingTop: '2em'}}>
        <Box style={{ position: 'absolute', top: '20%', left: '50%', transform: 'translate(-50%, -50%)'}}>
          <form>
            ...
            <Button class="btn" onClick={e => navigate(`/manufacturer/list`)}>Return</Button>
          </form>
        </Box>
      </Box>
    </Box>
